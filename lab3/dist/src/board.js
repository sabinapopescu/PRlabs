...
export class Board {
     ...
     */
    addChangeWatcher(playerId, resolver) {
     ...
     */
    rememberLingering(playerId, row, col) {
     ...
     */
    picturesDump() {
        const out = [`${this.rows}x${this.cols}`];
        for (let r = 0; r < this.rows; r++) {
            const cardsRow = this.cards[r];
            if (!cardsRow)
                continue;
     ...
     */
    registerPlayer(id, displayName = id) {
        if (id.length === 0 || /\s/.test(id))
            throw new Error('player id must be nonempty, no whitespace');
    ...
    async flipUp(playerId, row, col) {
        ...
        const faceUpRow = this.faceUp[row];
        const controllerRow = this.controller[row];
        if (!cardsRow || !faceUpRow || !controllerRow)
            throw new Error('invalid row');
     ...
     */
    async cleanupPreviousPlay(player) {
        const firstCard = player.getFirstCard();
     ...
     * @param col column index of the card (0-based)
     */
    flipDownIfUncontrolled(row, col) {
        const cardsRow = this.cards[row];
        const faceUpRow = this.faceUp[row];
        const controllerRow = this.controller[row];
        if (!cardsRow || !faceUpRow || !controllerRow)
